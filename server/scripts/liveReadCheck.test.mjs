...
describe('live read check command', () => {
  it('prints a v2 unified leaderboard without writes or provider calls', async () => {
    ...
    const read = vi.fn(async () => ({ rankingMode: 'unified', snapshot: { cycleId: 'cycle-v2', selectedWindow: '1Y', scoredAt: '2026-09-04T12:00:00.000Z', snapshotFormatVersion: 2 }, entries: [{ publicRank: 1, title: 'Unified topic', publicScore: 88, growthPercent: 184, growthSource: 'provider-history', evidenceStatus: 'emerging', trendHeat: 'stable', heatStatus: 'available', heatLevel: 'stable', heatEvidenceAvailable: true, heatEvidenceSource: 'current-intensity', heatFallbackUsed: true, heatPendingReason: null, movement: { state: 'new' }, componentAvailability: { presentation: { growthDiagnostics: { availability: true, fallbackReason: 'unsupported-window', promotion: { promotionOutcome: 'fallback' } } } } }] }))
    const write = vi.fn()
    ...
    expect(write.mock.calls.flat().join('\n')).toContain('promotion=fallback; fallback=unsupported-window')
    expect(write.mock.calls.flat().join('\n')).toContain('heat stable; status=available; evidence=available; source=current-intensity; fallback=yes; pending=none')
