...
async function readGrowthMeasurements({ repository, candidateIds, startAt, endAt, slotMinutes }) {
  if (repository.listLiveCanonicalAttentionPoints) {
    ...
    if (canonical.length) return { source: 'canonical-attention', measurements: canonicalMeasurements(canonical, 60), points: canonical, slotMinutes: 60 }
  }
...
function evaluateCanonical24hCoverage({ measurements, candidateIds, asOf, schedulerSlotMinutes }) {
  ...
  return new Map([...byCandidate].map(([candidateId, points]) => {
    ...
    if (!latest) return [candidateId, evaluateVaultGrowth({ measurements: points, window: '24H', asOf, slotMinutes: 60 })]
    ...
    const result = evaluateVaultGrowth({ measurements: points, window: '24H', asOf: latest.slot_at, slotMinutes: 60 })
...
export async function readHistoricalVaultCoverage({ repository, candidateIds, window, asOf = new Date().toISOString(), slotMinutes = 240 }) {
  ...
  const result = await readGrowthMeasurements({ repository, candidateIds, startAt: new Date(end - historyMs).toISOString(), endAt: new Date(end + 1).toISOString(), slotMinutes })
  if (result.source === 'canonical-attention' && window === '24H') {
    return evaluateCanonical24hCoverage({ measurements: result.measurements, candidateIds, asOf, schedulerSlotMinutes: slotMinutes })
  }
...
export async function readTopicHistoricalVault({ repository, candidateId, window, asOf = new Date().toISOString(), slotMinutes = 240 }) {
  ...
  const end = Date.parse(asOf)
  const result = await readGrowthMeasurements({ repository, candidateIds: [candidateId], startAt: new Date(end - RANGE_MS[window] * 2).toISOString(), endAt: new Date(end + 1).toISOString(), slotMinutes })
