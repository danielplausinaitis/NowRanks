...
export function assessDataForSeoBatchComparability(batchCount) {
  ...
  if (batchCount === 1) return { status: CROSS_QUERY_COMPARABILITY_STATUS.COMPARABLE, scope: 'single-batch', basis: 'Provider relative scale applies to the one submitted keyword set' }
  return { status: CROSS_QUERY_COMPARABILITY_STATUS.NOT_COMPARABLE, scope: 'multi-batch', basis: 'Provider documents per-request normalization but no cross-request calibration guarantee' }
}
...
export function assertGlobalDataForSeoComparable(batchCount) {
  ...
  if (assessment.status !== CROSS_QUERY_COMPARABILITY_STATUS.COMPARABLE) throw new Error('DataForSEO measurements from multiple keyword batches cannot be used for global scoring: cross-batch comparability is unresolved')
  return assessment
}
