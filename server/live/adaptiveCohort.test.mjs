...
const candidates = Array.from({ length: 20 }, (_, index) => ({ providerId: 'serpapi-google-trends-trending-now', query: `Topic ${index + 1}`, normalizedQuery: `topic ${index + 1}`, category: 'Technology', searchVolume: 100 - index, retrievedAt: '2026-09-05T00:00:00Z', geographicScope: scope }))
...
describe('adaptive live paid cohort', () => {
  ...
  it('persists 20 public rows when 20 current discoveries are selected and successfully measured beside retained tracking topics', async () => {
    const discovery = candidates.slice(0, 20)
    const retained = Array.from({ length: 30 }, (_, index) => ({ query: `Retained ${index}`, normalizedQuery: `retained ${index}`, category: 'Technology', trackingOnly: true, retrievedAt: '2026-09-05T00:00:00Z', geographicScope: scope }))
    const measure = vi.fn(async ({ keywords }) => ({ response: response(keywords), retrievedAt: '2026-09-05T00:00:00Z', task: {} }))
    const cycle = await collectLiveWindowCycle({
      sharedInputs: { ...shared(), candidates: discovery, volumes: volumes.slice(0, 20) }, paidTrackingCandidates: [...discovery, ...retained],
      historyRequest: {}, historyWindow: '24H', trendsMode: 'single', trendsClient: { measure }, scoreCycle: scorer(new Set(Array.from({ length: 20 }, (_, index) => index + 1))),
      displayLimit: 20, maxPaidCandidates: 50,
    })
    const plan = buildLivePersistencePlan({
      cycleId: '24h-full-public', historyWindow: '24H', scoredAt: '2026-09-05T00:00:00Z', displayLimit: 20, candidates: cycle.candidates, volumes: cycle.volumes, histories: cycle.histories,
      scores: cycle.scores.map((entry) => ({ ...entry, nowScore: entry.unifiedRawScore, components: { searchInterest: 1 }, componentDiagnostics: {}, history: { observationCount: 1, availableCount: 1, coveragePercentage: 100 }, topicClassification: 'established', confidence: 'full', confidenceReason: 'test evidence', evidenceStatus: 'established' })),
    })
    expect(cycle.requestMetrics.evaluation).toMatchObject({ discoveryCandidateCount: 20, scoringDiscoveryCandidateCount: 20, selectedPaidTrackingCount: 50, selectedButNotMeasuredCount: 0 })
  ...
  it('rejects a cohort below the configured public display limit instead of persisting a partial board', () => {
    ...
    expect(plan.publication).toMatchObject({ publishable: false, requiredCount: 20, availableCount: 12, reason: 'insufficient-public-candidates' })
