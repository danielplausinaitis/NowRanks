...
export function App({ useLeaderboardApi = import.meta.env.VITE_USE_LEADERBOARD_API === 'true', apiClient = fetchLeaderboard }: { useLeaderboardApi?: boolean, apiClient?: LeaderboardLoader }) {
  ...
  useEffect(() => {
    ...
    setApiState('loading')
    void apiClient({ window, mode: rankingMode, ...(category === 'All' ? {} : { category }), signal: controller.signal })
      .then((result) => {
        if (version !== requestVersion.current) return
  ...
  }, [apiClient, category, rankingMode, retryCount, scoreMode, useLeaderboardApi, window])
  const categories = useMemo(() => ['All', ...CATEGORIES] as const, [])
  const visibleEntries = entries
  const sourceLabel = apiMetadata
    ? `${apiMetadata.dataMode.toUpperCase()}${apiMetadata.dataMode === 'replay' ? ' — NOT LIVE GOOGLE DATA' : ''} · ${apiMetadata.providerId} · ${apiMetadata.mode} · Observed through ${apiMetadata.observedThrough}`
