...
## Persisted-live public UI

The rankings page requests only the persisted unified v2 live snapshot from `/api/leaderboard`; it never renders replay candidates or locally calculated ranks. Start the read-only API with `npm run api:dev`, then start Vite with `npm run dev`. Browsing this path does not trigger ingestion, provider requests, or writes.
